...
namespace Web_API.Services
{
    public class TreeService : BaseCrudService<Tree>
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(treeId) && Context.Trees.Any(c => c.Id == new Guid(treeId) && c.Name == name && c.IdTreeCategory == categoryId && c.IsActive))
                return false;
            else
                return Context.Trees.Any(c => c.Name == name && c.IdTreeCategory == categoryId && c.IsActive);
        }
